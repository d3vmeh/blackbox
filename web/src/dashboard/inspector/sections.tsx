...
export function Section({ title, aside, children }: {
  ...
  return (
    <div className="insp__sec">
      <div className="insp__sech">
        <span className="insp__sectitle">{title}</span>
        {aside && <span className="insp__secaside" title={aside}>{aside}</span>}
      </div>
      {children}
    </div>
  )
...
export function Field({ k, v, tone }: { k: string; v: React.ReactNode; tone?: 'bad' | 'good' | 'root' }) {
  return (
    <div className="insp__kv">
      <span className="insp__k" title={k}>{k}</span>
