...
describe('useRun', () => {
  it('exposes the coding-run trace, attribution, and a derived graph', () => {
  ...
  it('replays: root step (s1) flips, decoy (s3) does not', async () => {
    const { result } = renderHook(() => useRun())
    let root!: Awaited<ReturnType<typeof result.current.replay>>
    let decoy!: Awaited<ReturnType<typeof result.current.replay>>
    await act(async () => { root = await result.current.replay('s1', { unit: 'seconds' }) })
    await act(async () => { decoy = await result.current.replay('s3', true) })
