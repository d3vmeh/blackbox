"""Arize AX tracing — LLM auto-instrumentation + manual agent/tool spans."""

from __future__ import annotations

import os
from typing import TYPE_CHECKING

if TYPE_CHECKING:
    from opentelemetry.sdk.trace import TracerProvider

_tracer_provider: TracerProvider | None = None
_INSTRUMENTED = False


def setup_tracing() -> None:
    """Register OpenTelemetry with Arize AX. Call before creating the Anthropic client."""
    global _INSTRUMENTED, _tracer_provider
    if _INSTRUMENTED:
        return

    space_id = os.environ.get("ARIZE_SPACE_ID")
    api_key = os.environ.get("ARIZE_API_KEY")
    if not space_id or not api_key:
        raise RuntimeError(
            "Set ARIZE_SPACE_ID and ARIZE_API_KEY in .env "
            "(Space Settings → API Keys at https://app.arize.com)"
        )

    project_name = os.environ.get("ARIZE_PROJECT_NAME", "fuse-breaker")

    from arize.otel import register
    from openinference.instrumentation.anthropic import AnthropicInstrumentor

    _tracer_provider = register(
        space_id=space_id,
        api_key=api_key,
        project_name=project_name,
    )
    AnthropicInstrumentor().instrument(tracer_provider=_tracer_provider)

    _INSTRUMENTED = True
    print(f"Arize AX tracing on · project={project_name}")
    print(f"  space_id={space_id[:12]}...")


def flush_tracing(timeout_ms: int = 15_000) -> None:
    """Export pending spans before a short-lived CLI exits."""
    if _tracer_provider is not None and hasattr(_tracer_provider, "force_flush"):
        _tracer_provider.force_flush(timeout_millis=timeout_ms)


def get_tracer(name: str = "fuse-breaker"):
    from opentelemetry import trace

    return trace.get_tracer(name, "1.0.0")
