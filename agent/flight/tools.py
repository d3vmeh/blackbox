...
Browserbase track), navigates the live web, and scrapes prices. On ANY failure it falls
back to `mock_browserbase_search` so the agent pipeline never breaks.

Both return: list[{"flight": str, "depart": str, "price": int}]. `depart` echoes the
requested date (that's the date we searched for).
"""
...
def mock_browserbase_search(dest: str, date: str) -> list[dict]:
    """Deterministic offline results — no account needed. UA-441 is the cheapest."""
    return [
...
def _browserbase_search(dest: str, date: str, max_results: int) -> list[dict]:
    ...
    with sync_playwright() as p:
        browser = p.chromium.connect_over_cdp(connect_url)  # remote Browserbase browser
